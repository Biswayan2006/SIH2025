...
export default function Navbar() {
  ...
  const linkClass = ({ isActive }) =>
    `relative font-medium px-3 py-1 transition-all duration-200 outline-none transform
     ...
     focus:outline-none focus:-translate-y-0.5 rounded-md`
    ...
    <>
      ...
      <nav className="fixed top-0 w-full bg-[#F8F9FA]/70 dark:bg-[#1a1a2e]/90 backdrop-blur-md shadow-lg flex justify-between items-center px-4 py-2 z-50 transition">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <LanguageSelector />
          <ThemeToggle />
          <NavLink to="/login" className="hidden sm:inline-block bg-cyan-500 hover:bg-cyan-600 text-white px-2 py-1 text-sm rounded-lg font-medium shadow-md hover:shadow-cyan-400/40 transition-all duration-200 transform hover:-translate-y-0.5 outline-none focus:-translate-y-0.5">
            {translate('login')}
          </NavLink>
        </div>
      ...
      >
        <div className="flex flex-col mt-20 px-4">
          ...
          <div className="flex flex-col items-center mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
            ...
            <NavLink to="/login" className="mt-2 bg-[#1B9AAA] hover:bg-cyan-600 text-white px-6 py-2 rounded-lg font-medium shadow-md hover:shadow-cyan-400/40 transition-all duration-200 transform hover:-translate-y-0.5 outline-none focus:-translate-y-0.5">
              {translate('login')}
            </NavLink>
          </div>
            ...
            {navItems.map(item => {
              ...
              return (
                <NavLink 
                  ...
                  className={({ isActive }) => 
                    `flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 transform ${isActive ? 'text-cyan-600 dark:text-cyan-400 -translate-y-0.5' : 'text-gray-800 dark:text-white hover:text-cyan-500 hover:-translate-y-0.5'}`
                  }
                >
                  {getIcon(item.to)}
